...
class PpfPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ppf.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ppf`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ppf WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ppf (date, title, link, content)"
